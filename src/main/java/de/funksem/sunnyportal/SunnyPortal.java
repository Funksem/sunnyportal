...
public class SunnyPortal
{
    ...
    static
    {
        ...
        options.addOption("m", "guthaben", true, "Feste Einnahmen pro Monat");
    }
    ...
    private static void callRightMethod(CommandLine cli) throws FileNotFoundException, IOException
    {
        ...
        final String strGuthabenProMonat = cli.getOptionValue('m');
        ...
        Double guthabenProMonat = null;
        if (!StringUtils.isBlank(strGuthabenProMonat))
        {
            guthabenProMonat = ConverterUtils.toDouble(strGuthabenProMonat);
        }

        SunnyPortalExecutor executor = new SunnyPortalExecutor();
        executor.start(sourceDirectory, verguetungInEuro, guthabenProMonat);
    }
